...
void GameHelper::checkAndSet(std::unique_ptr<Game> &game, const int firstPlayer, const int secondPlayer, const int maxPlayer) {
	...
	switch(game->tableCard().CT) {
		...
		case CardType::REVERSE:
			if (maxPlayer > 2) {
				if (game->direction() == DIRECTION::CLOCKWISE) game->direction(DIRECTION::COUNTERCLOCKWISE);
				else game->direction(DIRECTION::CLOCKWISE);
		...
		case CardType::SKIP:
			if (maxPlayer > 2) game->state(PlayerState::BREAK, secondPlayer);
			else game->state(PlayerState::CONTINUE, firstPlayer);
			break;
...
/*
	Return the amount of points for a CardType for the AI.

	TODO: Change some points or so?
*/
...
int GameHelper::getHighestCard(std::unique_ptr<Game> &currentGame, const int player) {
	...
	for (int i = 0; i < currentGame->getSize(player); i++) {
		if (currentGame->Playable(i, player)) index.push_back({ GameHelper::AIPointTable(currentGame->getType(i, player)) });
